...
class TestBevorApiClientLive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not (os.getenv("BEVOR_LIVE_TESTS", "false").lower() in ("1", "true", "yes")):
            self.skipTest("Live tests disabled. Set BEVOR_LIVE_TESTS=true to run.")
        ...
        # Ensure API key is present for auth
...
contract BevorToken is ERC20 {
    ...
    async def _client_integration_async(self):
        ...
        try:
            client = await BevorApiClient(
                bevor_api_key=api_key,
                contracts_folder_path=str(self.contracts_dir),
                project_id="ee5aaf7e-71a5-4c2d-8b20-96af42610367",
            ).create()
